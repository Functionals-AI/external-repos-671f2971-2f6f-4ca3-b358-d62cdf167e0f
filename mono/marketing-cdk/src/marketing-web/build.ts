...
export function createBuildProject(stack: Construct, options: CreateBuildOptions): CreateBuildResult {
  ...
  const buildType = config.isDevelopment ? 'dev' : 'prod'
      ...
      phases: {
        ...
        build: {
          commands: [
            ...
            `DOCKER_BUILDKIT=1 docker build -t mono/${name} -f ./${name}/Dockerfile ${buildContext} --build-arg BUILD_TYPE=${buildType} ${Object.keys(env||{}).map(key => `--build-arg ${key}=\${${key}}`).join(' ')}`,
