...
const MTAG = ['telenutrition-e2e', 'browser', 'telenutrition', 'member', 'schedule', 'sms']
const SCHEDULE_METHOD = E2EScheduleVerificationMethods.SMS

/**
 * This function is responsible for end-to-end testing of appointment scheduling via SMS
 ...
 */
export async function e2EScheduleSMS(context: IContext, e2EContext: E2EBrowserContext): Promise<Result<void, ErrCode>> {
  const { logger, config } = context
  const { page, baseUrl } = e2EContext
  const TAG = [...MTAG, 'e2EScheduleSMS']
  ...
  try {
    ...
    logger.info(context, TAG, `memberConfig: ${JSON.stringify(memberConfig)}`)
    await page.goto(url)
